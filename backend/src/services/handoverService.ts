...
export async function getHandoverPreviewByToken(input: { pickupToken: string }) {
  ...
  return {
    ...
    item: {
      ...
      color: claim.foundItem.color,
      foundLocation: claim.foundItem.foundLocation,
      foundAtUtc: claim.foundItem.foundAtUtc.toISOString(),
      privateDiscoveryNote: claim.foundItem.privateDiscoveryNote,
      storageLocation: claim.foundItem.storageLocation ?? "Unassigned",
      status: claim.foundItem.status,
      photoUrl: claim.foundItem.privateData && typeof claim.foundItem.privateData === 'object' && 'photoUrl' in (claim.foundItem.privateData as object) ? (claim.foundItem.privateData as Record<string, string>).photoUrl : undefined,
    },
  };
}
